...
const CriticalIncidentsList = () => {
  ...
  const [activePriority, setActivePriority] = useState<PriorityValue | null>(
    initialAccordionValue
  );

  // Search terms and sort configs must now be keyed by PriorityCode OR the ALL_PRIORITIES_KEY
  const initialSearchTerms: Record<
    PriorityCode | typeof ALL_PRIORITIES_KEY,
    string
  > = { [ALL_PRIORITIES_KEY]: "" };
  ...
  dynamicPriorityCodes.forEach(
    (code) => (initialSortConfigs[code] = { key: "id", direction: "asc" })
  );
  ...
  const getProcessedIncidents = useMemo(() => {
    ...
    return processedData as Record<
      PriorityCode | typeof ALL_PRIORITIES_KEY,
      Incident[]
    >;
  ...
  const handleAccordionChange = (value: string | null) => {
    setActivePriority(
      value as PriorityValue | typeof ALL_PRIORITIES_KEY | null
    );
    setCurrentPage(1);
  };

  const handleSearchChange = (
    key: PriorityCode | typeof ALL_PRIORITIES_KEY,
    term: string
  ) => {
  ...
  const handlePageChange = (newPage: number) => {
    const activeCode =
      activePriority === ALL_PRIORITIES_KEY
        ? ALL_PRIORITIES_KEY
      ...
      <div className="filter-tabs">
        <button
          className={`tab-button ${
            activePriority === ALL_PRIORITIES_KEY ? "active" : ""
          }`}
...
const IncidentAccordionItem: React.FC<IncidentAccordionItemProps> = ({
  ...
  const paginatedIncidents =
    activePriority === priorityValue ? allIncidents.slice(start, end) : [];
  ...
  const renderSortIcon = (key: SortKey) => {
    ...
    return (
      <span className="sort-indicator default">
        <BsSortUp size={14} />
      </span>
    );
  };
            ...
            <div className="incident-table-container">
              ...
              <div className="incident-row header-row">
                <span
                  ...
                  onClick={() => handleSort("id", dataKey)}
                >
                  Incident no {renderSortIcon("id")}
                </span>
                <span
                  className="col-description"
                  onClick={() => handleSort("description", dataKey)}
                >
                  Description {renderSortIcon("description")}
                </span>
                <span
                  className="col-category"
                  onClick={() => handleSort("category", dataKey)}
                >
                  Category {renderSortIcon("category")}
                </span>
                <span
                  className="col-resolution"
                  onClick={() => handleSort("resolution", dataKey)}
                >
                  Resolution notes{renderSortIcon("resolution")}
                </span>
                <span
                  className="col-state"
                  onClick={() => handleSort("state", dataKey)}
                >
                  State {renderSortIcon("state")}
                </span>
                <span
                  className="col-resolved-date"
                  onClick={() => handleSort("resolvedDateTime", dataKey)}
                >
                  Resolved <br /> Date & Time{" "}
                  {renderSortIcon("resolvedDateTime")}
                </span>
              </div>
      ...
      <div>
        ...
        {activePriority === priorityValue && totalItems > 0 && (
          <div className="pagination-footer">
            <span>{`Showing ${Math.min(start + 1, totalItems)}-${Math.min(
              end,
              totalItems
            )} of ${totalItems} Total ${dataKey} Incidents`}</span>
